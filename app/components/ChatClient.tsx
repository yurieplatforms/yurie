...
export default function ChatClient() {
  ...
  // Removed firstAssistantChunkMs logic; we now rely on visible content detection
  ...
  // Store active tab per user message index
  const [activeTabByMessageIndex, setActiveTabByMessageIndex] = useState<Record<number, 'AI Mode' | 'All' | 'Images' | 'Videos' | 'News'>>({})
  // Store search data per user message index
  ...
  // Determine whether the assistant message has any visible content yet
  function hasVisibleAssistantBubbleContent(content: string): boolean {
  ...
  const handleSendMessage = useCallback((message: string, uploadedFiles?: File[], options?: { reset?: boolean }) => {
    ...
    // When starting a fresh chat from an existing message, clear related state
    ...
    setStatus('submitted')
    // no-op: we show shimmer until visible content arrives
    ...
    async function processMessage() {
      try {
        ...
        const userMessageIndex = nextMessages.length - 2 // User message is second to last
        const searchPromise: Promise<any | null> = (async () => {
          if (!useSearchMode) return null
          try {
            setIsFetchingSearch(true)
            const usp = new URLSearchParams({ q: sanitizedUser, hl: 'en', gl: 'us', google_domain: 'google.com', safe: 'active', num: '10' })
        ...
        // assistantIndex is already at nextMessages.length - 1 since we added it earlier
        const assistantIndex = nextMessages.length - 1
        setReasoningByMessageIndex((prev) => ({ ...prev, [assistantIndex]: '' }))

        // Assistant message already added in nextMessages, no need to add again
        setStatus('streaming')
        ...
        while (true) {
          ...
          // Shimmer visibility now based on hasVisibleAssistantBubbleContent via render path
      ...
      >
        {messages.length === 0 ? null : (
          ...
          {messages.map((m, i) => {
            ...
            const hasVisible = hasVisibleAssistantBubbleContent(m.content || '')
            ...
            return (
              <div key={i} className={`${topMarginClass} mb-0`}>
                {/* Show user message with tabs container for every user message with an assistant response */}
                  ...
                  <>
                    <div className="pt-2 pb-5 border-b border-neutral-300 dark:border-neutral-700 mb-2 mt-2">
                      ...
                      <div className="mb-3 -mx-2 sm:-mx-4">
                        <div className={cn('w-full flex items-center gap-0 justify-end')}>
                          <div
                            className={cn(
                              'min-w-0 w-full max-w-full pl-2 sm:pl-4 pr-4 sm:pr-6 break-words py-2.5 text-base leading-snug flex-1',
                      ...
                      <div className="flex items-center gap-6 sm:gap-8 flex-wrap mt-6">
                        {(['AI Mode','All','Images','Videos','News'] as const).map((tab) => (
                          <button
                            key={tab}
                            className={cn(
                              'text-[15px] sm:text-base transition cursor-pointer focus:outline-none focus-visible:outline-none',
                              activeTab === tab
                                ? 'text-neutral-900 dark:text-white font-semibold'
                                : 'text-neutral-700 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-neutral-100'
                            )}
                            onClick={() => setActiveTabByMessageIndex((prev) => ({ ...prev, [i]: tab }))}
                          >
                            {tab}
                          </button>
                        ))}
                    ...
                    {activeTab !== 'AI Mode' && (
                      <div className="mt-6 mb-2">
                        ...
                        <SearchResults data={searchData} section={activeTab as any} />
                      </div>
                    )}
                  </>
                )}
                {/* Render message content - skip user messages that are shown in tabs */}
                {m.role === 'user' ? (
                  // Only show standalone user message if it doesn't have tabs (no assistant response after it)
                  !isUserWithAssistantResponse && (
                    <div className={cn('w-full flex items-center gap-0 justify-end -mx-2 sm:-mx-4')}>
                      <div
                        className={cn(
                          'min-w-0 w-full max-w-full px-2 sm:px-4 break-words py-0.5 text-base leading-snug flex-1',
                          'text-neutral-900 dark:text-white',
                          'bg-white dark:bg-[#303030] border border-gray-200 dark:border-[#444444]'
                        )}
                        style={{ borderRadius: 32, boxShadow: "0 2px 8px 0 rgba(0,0,0,0.08)" }}
