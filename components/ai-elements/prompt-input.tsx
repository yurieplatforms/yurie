...
export const PromptInput = ({
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    const el = (form.elements.namedItem("message") as
      ...
      | (HTMLTextAreaElement & { value?: string })
      | null) as any;
    const textValue = (el && typeof el.value === "string" ? el.value : "") as string;

    onSubmit({ text: textValue, files }, event);

    try {
      // Clear local attachments managed by this component
      clear();
      // Reset the form to clear the textarea value and any native inputs
      form.reset();
    } catch {}
  };
...
export const PromptInputModelSelectTrigger = ({
  ...
  <SelectTrigger
    className={cn(
      "border-none bg-transparent font-medium text-foreground/80 shadow-none transition-colors",
