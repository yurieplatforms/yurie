...
export async function saveTracks(params: SaveTracksParams): Promise<ComposioToolOutput> {
  return executeSpotifyTool('SPOTIFY_SAVE_TRACKS_FOR_CURRENT_USER', {
    ids: params.ids.map(cleanSpotifyId),
  })
}
...
/**
 * Clean Spotify ID by removing URI prefix if present.
 * e.g. "spotify:track:123" -> "123"
 */
function cleanSpotifyId(id: string): string {
  return id.replace(/^spotify:[a-z]+:/, '')
}
