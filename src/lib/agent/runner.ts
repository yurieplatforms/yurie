...
function sanitizeContentBlock(block: Anthropic.ContentBlockParam): Anthropic.ContentBlockParam {
  ...
  switch (type) {
    case 'text': {
      const textBlock = block as { type: 'text'; text: string; cache_control?: { type: 'ephemeral' }; parsed?: unknown }
        ...
        for (let i = content.length - 1; i >= 0; i--) {
          ...
          if ('type' in block && (block.type === 'text' || block.type === 'image' || block.type === 'document' || block.type === 'tool_result')) {
            ...
            if (sanitized && typeof sanitized === 'object') {
              // Explicitly remove 'parsed' field if it somehow survived (e.g. from structured outputs)
              if ('parsed' in sanitized) {
                delete (sanitized as { parsed?: unknown }).parsed
              }
