...
import type { MemoryToolHandler, MemoryToolInput } from '@/lib/tools/memory'
...
export function createMemoryTool(
  ...
  return {
    ...
    run: async ({ input }: { input: MemoryToolInput }) => {
      await sseHandler.sendToolEvent('memory', 'start', input as Record<string, unknown>)
      
      try {
        if (!memoryToolHandler) {
          const errorMsg = 'Memory tool requires authentication. Please log in to use persistent memory.'
          await sseHandler.sendToolEvent('memory', 'end', input as Record<string, unknown>, errorMsg)
          return errorMsg
        }

        const result = await memoryToolHandler.execute(input)
        
        if (result.success) {
          await sseHandler.sendToolEvent('memory', 'end', input as Record<string, unknown>, result.content)
