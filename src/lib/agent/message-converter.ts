...
export function convertToAnthropicContent(
  ...
  if (Array.isArray(content)) {
    ...
    // Ensure we don't return empty content which causes API errors
    if (result.length === 0) {
      // If we had input segments but result is empty (e.g. unsupported files),
      // return a placeholder to prevent 400 Bad Request
      if (content.length > 0) {
        return '[Unsupported or empty content]'
      }
      // If input was truly empty, return empty string (likely to be caught by other validation)
      return ''
    }
