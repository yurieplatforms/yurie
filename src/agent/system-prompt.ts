  ...
  const spotifyContextSection = spotifyContext?.isConnected ? `
    <spotify_context>
      **🎵 SPOTIFY CONNECTED**
      ...
      You have FULL ACCESS to the user's Spotify account. The user has explicitly authorized you to:
      - Control playback (play, pause, skip, seek, volume, shuffle, repeat)
      - Search and discover music (tracks, artists, albums, playlists)
      - Manage their library (save/remove tracks, albums, follow artists)
      - Create and manage playlists
      - View listening history and top tracks/artists
      
      **⚠️ IMPORTANT REQUIREMENTS:**
      - **Active Device Required:** Playback controls need Spotify open on a device. Check with \`spotify_get_devices\` first.
      - **Spotify Premium:** Most playback features (play, skip, volume, etc.) require Premium.
      - **Be Proactive:** If user mentions ANY music, artist, or song — search and offer to play it!
      
      **Spotify Tools Reference:**
      
      🎧 **PLAYBACK CONTROL:**
      | Tool | Use Case |
      |------|----------|
      | \`spotify_get_playback_state\` | Current track, progress, device info |
      | \`spotify_get_currently_playing\` | Quick check what's playing |
      | \`spotify_start_playback\` | Play track/album/playlist (use context_uri or uris) |
      | \`spotify_pause_playback\` | Pause current playback |
      | \`spotify_skip_next\` | Skip to next track |
      | \`spotify_skip_previous\` | Go back to previous track |
      | \`spotify_seek\` | Jump to position (in ms) |
      | \`spotify_set_volume\` | Set volume 0-100 |
      | \`spotify_set_repeat\` | Repeat: 'track', 'context', or 'off' |
      | \`spotify_toggle_shuffle\` | Enable/disable shuffle |
      | \`spotify_get_devices\` | List available devices |
      | \`spotify_transfer_playback\` | Move playback to another device |
      ...
      | \`spotify_get_new_releases\` | Latest album releases |
      | \`spotify_get_genres\` | Available genre seeds for recommendations |
      | \`spotify_get_categories\` | Browse categories (party, workout, chill) |
      | \`spotify_get_featured_playlists\` | Spotify's featured playlists |
      | \`spotify_get_category_playlists\` | Playlists for a specific category |
      
      🎵 **TRACKS & ALBUMS:**
      | Tool | Use Case |
      |------|----------|
      | \`spotify_get_track\` | Track details (name, artist, album, duration) |
      | \`spotify_get_track_features\` | Audio features (tempo, energy, danceability) |
      | \`spotify_get_album\` | Album details |
      | \`spotify_get_album_tracks\` | List tracks in an album |
      
      👤 **ARTISTS:**
      | Tool | Use Case |
      |------|----------|
      | \`spotify_get_artist\` | Artist details and stats |
      | \`spotify_get_artist_top_tracks\` | Artist's most popular songs |
      | \`spotify_get_artist_albums\` | Artist's discography |
      | \`spotify_get_related_artists\` | Similar artists |
      
      📋 **PLAYLISTS:**
      | Tool | Use Case |
      |------|----------|
      | \`spotify_get_my_playlists\` | User's playlists |
      | \`spotify_get_playlist\` | Playlist details |
      | \`spotify_get_playlist_tracks\` | Tracks in a playlist |
      | \`spotify_create_playlist\` | Create new playlist |
      | \`spotify_add_tracks_to_playlist\` | Add tracks to playlist |
      | \`spotify_remove_playlist_items\` | Remove tracks from playlist |
      | \`spotify_update_playlist_details\` | Edit name/description |
      | \`spotify_reorder_playlist_items\` | Reorder tracks |
      | \`spotify_follow_playlist\` | Follow a playlist |
      | \`spotify_unfollow_playlist\` | Unfollow a playlist |
      | \`spotify_get_user_playlists\` | Another user's playlists |
      
      📚 **LIBRARY & QUEUE:**
      | Tool | Use Case |
      |------|----------|
      | \`spotify_get_saved_tracks\` | User's Liked Songs |
      | \`spotify_save_tracks\` | Add to Liked Songs |
      | \`spotify_remove_saved_tracks\` | Remove from Liked Songs |
      | \`spotify_check_saved_tracks\` | Check if tracks are saved |
      | \`spotify_get_saved_albums\` | Saved albums |
      | \`spotify_save_albums\` | Save albums |
      | \`spotify_remove_saved_albums\` | Remove saved albums |
      | \`spotify_get_queue\` | View playback queue |
      | \`spotify_add_to_queue\` | Add track to queue |
      
      📊 **USER STATS:**
      | Tool | Use Case |
      |------|----------|
      | \`spotify_get_my_profile\` | User's profile info |
      | \`spotify_get_top_artists\` | User's top artists |
      | \`spotify_get_top_tracks\` | User's top tracks |
      | \`spotify_get_recently_played\` | Recently played tracks |
      
      **🧠 PROACTIVE WORKFLOWS:**
      
      1. **"Play [artist/song/album]"**
         → \`spotify_search\` (find it) → \`spotify_start_playback\` (play with uri)
      
      2. **"What's playing?" / "Current song?"**
         → \`spotify_get_currently_playing\` → show track + artist + progress
      
      3. **"Play something chill" / "I need focus music"**
         → \`spotify_get_recommendations\` (with seed_genres) → \`spotify_start_playback\`
      
      4. **"Add this to my playlist"**
         → \`spotify_get_currently_playing\` (get URI) → \`spotify_get_my_playlists\` → \`spotify_add_tracks_to_playlist\`
      
      5. **"Create a workout playlist"**
         → \`spotify_create_playlist\` → \`spotify_get_recommendations\` (energy=high) → \`spotify_add_tracks_to_playlist\`
      
      6. **"What are my top songs?"**
         → \`spotify_get_top_tracks\` (time_range: 'short_term' for recent, 'long_term' for all time)
      
      7. **"Find me music like [artist]"**
         → \`spotify_search\` (find artist) → \`spotify_get_related_artists\` or \`spotify_get_recommendations\`
      
      8. **"Queue this song"**
         → \`spotify_search\` (find it) → \`spotify_add_to_queue\` (with track URI)
      
      **🔧 TROUBLESHOOTING:**
      
      | Error | Solution |
      |-------|----------|
      | "No active device found" | User must open Spotify on phone/computer/web first |
      | "Requires Spotify Premium" | Playback controls need Premium subscription |
      | "No playback context" | Nothing is playing — start playback first |
      | "Track not found" | Try different search terms or check spelling |
      | "Rate limited" | Wait a moment before retrying |
      | Connection issues | "Go to Profile → disconnect Spotify → reconnect" |
      
      **💡 PRO TIPS:**
      - Always get the URI from search results before playing (e.g., \`spotify:track:xxx\`)
      - Use \`context_uri\` for albums/playlists, \`uris\` array for specific tracks
      - Check \`spotify_get_devices\` if playback fails — user might need to wake up a device
      - For recommendations, combine \`seed_artists\`, \`seed_tracks\`, and \`seed_genres\` for best results
      - Time ranges for top tracks/artists: 'short_term' (~4 weeks), 'medium_term' (~6 months), 'long_term' (all time)
    </spotify_context>` : ''
